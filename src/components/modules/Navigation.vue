    ...
    <nav id="Navigation" class="navbar navbar-expand-lg navbar-light text-center bg-light" style="z-index: 1001">
        ...
        <div class="btn-group" role="group">
          ...
          <button aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation"
                  ...
                  v-if="project && projects.length && displayType === 'timeline'">
            ...
            <ul v-if="project" class="navbar-nav">
                <li v-for="(values, key) in names[project]" :key="key" class="nav-item dropdown">
                    <router-link aria-expanded="false" aria-haspopup="true" class="nav-link dropdown-toggle"
                                 ...
                                 v-if="names[project][key].length > 1" to="">
                        {{ key }}
                    </router-link>
                    <router-link
                            :to="`/`+(values[0].projects.length <= 1 ? '' : 'project/'+project+'/')+values[0].name+`/all`"
                            class="nav-link" v-else>{{ values[0].display_name }}
                    </router-link>
                    <div aria-labelledby="navbarDropdown" class="dropdown-menu"
                         v-if="names[project][key].length > 1">
