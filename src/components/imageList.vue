<template>
    <div id="imageList">
        ...
        <div v-else :class="(bottom ? '' : 'card')">
            <el-image :class="`rounded card-img` + (bottom ? '-bottom' : '-top')" v-for="(media, s) in list" :key="media.tweet_id+s" :src="basePath+`/api/v2/media/tweets/`+media.url" lazy fit="cover" :preview-src-list="previewList" :preview-src-list-order="s" :alt="media.uid+'_'+media.tweet_id+'_'+s">
                <div slot="error" class="image-slot"></div>
                <div slot="placeholder" class="image-slot" >
                    <!--此处待新操作 //TODO-->
