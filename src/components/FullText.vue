...
const emojiObject = (text: string = ''): {expanded_url: string; indices_start: number, indices_end: number; text: string; type: string}[] => parse(text, {buildUrl: buildUrl, assetType: 'svg'}).map(x => ({
  expanded_url: x.url,
