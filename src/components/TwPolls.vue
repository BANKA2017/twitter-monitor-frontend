  ...
  <div id="twPolls" class="my-2">
    <div class="container no-gutters text-decoration-none">
      <div class="row">
        ...
        <div v-if="(etaSeconds <= 0 && ((!settings.onlineMode && polls[0].checked)) || ((settings.onlineMode && polls.length)) && pollCount > 0)" class="col-12">
          <div class="progress" v-for="(poll, index) in polls" :key="poll.poll_order">
            <div :style="{'background-color': maxPollIndex === index ? '#7CC5F6' : '#CFD9DE',  width: Math.ceil((poll.count/pollCount)*100) + '%'}" class="progress-bar"></div>
            <div class="progress-data">
              <span :class="{'fw-bold': maxPollIndex === index, 'fw-normal': maxPollIndex !== index}">{{ poll.choice_label }}</span>
              <span :class="{'fw-bold': maxPollIndex === index, 'fw-normal': maxPollIndex !== index}">{{ Math.ceil((poll.count/pollCount)*100) + '%' }}</span>
            </div>
          </div>
        </div>
        <template v-else>
...
import {Media as MediaInterface, PollItem} from "@/type/Content";
