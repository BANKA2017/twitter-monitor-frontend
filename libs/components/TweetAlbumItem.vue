<template>
  <div id="tweet-album">
    <div class="container container-fluid mb-2">
      <div class="row">
        <router-link :to="`/i/status/${mediaItem.tweet_id}`" class="col-6 col-lg-4 px-1 d-block position-relative" v-for="(mediaItem, index) in mediaList" :key="index">
          <div style="top: 0.5em; right: 0.5em; " class="position-absolute" v-if="mediaItem.original_type !== 'photo'"><camera-video-icon height="2em" status="text-white" width="2em"/></div>
    ...
    <!--<div class="position-absolute list-group" :style="{top: state.position.y + 'px', left: (state.position.x + 200 > width ? state.position.x - 200 : state.position.x) + 'px'}" v-if="mediaList.length && state.position.index > -1" v-show="state.position.x >= 0 && state.position.y >= 0">
      ...
      <a :href="createRealMediaPath(realMediaPath, samePath, 'tweets')+mediaList[state.position.index].url + (mediaList[state.position.index].original_type !== 'photo' ? '' : ':orig')" class="list-group-item list-group-item-action" target="_blank">下載{{mediaList[state.position.index].original_type !== 'photo' ? '視頻' : '原圖'}}</a>
