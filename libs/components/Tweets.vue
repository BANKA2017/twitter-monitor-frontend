...
import {TweetType} from "../types/State";
import {TweetMode} from "../types/State";
...
import TweetAlbumItem from "./TweetAlbumItem.vue";
...
const routeCase = (to: RouteLocationNormalized) => {
  ...
  switch (to.name) {
    ...
    case "communities":
        url += 'tweets/'
        queryStringObject.set('community_id', <string>NullSafeParams(to.params.communityId, ''))
        queryStringObject.set('count', '20')
        setTweetMode('timeline')
        break
    default:
...
const routeRun = (to: RouteLocationNormalized, from: RouteLocationNormalized | {name: string, params: {[p in string]: string | number | boolean}}) => {
  ...
  routeCase(to)
  emptyTranslateList()
  if (!(to.name === 'search' && !to.query.q && to.query.advanced !== '1') || to.name === 'lists' || to.name === 'communities') {
...
router.afterEach((to, from) => {
  //if (!settings.value.onlineMode || to.name !== 'name-display') {
    routeRun(to, from)
  //}
})
watch(userInfo, () => {
