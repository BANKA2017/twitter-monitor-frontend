      ...
      <div class="row">
        <div class="col-lg-2 mb-4">
          <div class="d-grid gap-2" style="position: sticky; top: 1.5rem">
            ...
            <!--<button class="btn btn-outline-primary" @click="H2C(overview, 'bangdream_trends.png')">保存图片</button>-->
            <a :href="store.getters.getBasePath + '/static/bangdream_trends/' + state.status.date + '.json'" class="btn btn-outline-primary" target="_blank">下载数据 <download-icon height="1em" status="" width="1em" /></a>
          </div>
        </div>
        <div class="col-lg-8 mb-4">
          <div class="card" v-show="state.status.displayTips">
            <div class="card-body">
              ...
              <ul class="card-body">
                ...
                <li>本页仅提供 BanGDream! 企划中的 Aqours、虹ヶ咲学園 以及 Liella! 相关人士的数据，不包含 μ's</li>
          ...
          <template v-else>
            <div v-if="state.status.value === 'overview'" ref="overview">
              <single-page-header title="BanGDream! Trends" :sub-title="state.trendsData.range.start ? (createDate(state.trendsData.range.start) + '~' + createDate(state.trendsData.range.end)) : ''" padding="5vw" />
              ...
              <el-table class="mb-2" ref="accountData" v-loading="!state.trendsData.data.length" :data="tableData.sort((a, b) => b.followers_add - a.followers_add)" :default-sort="{prop: 'followers_add', order: 'descending'}" style="cursor: pointer; width: 100%" @row-click="(row) => {ScrollTo();$router.push('/i/topics/bangdream_trends/' + row.name)}">
                ...
                <el-table-column label="组" prop="team" style="">
                  <template #default="scope">
                    <div :style="{'background-color': color[scope.row.team], padding: '5px 11px', 'font-size': '12px', 'border-radius': '3px', 'word-break': 'break-word', }" class="text-white">
                      {{ scope.row.team }}
                    </div>
                  </template>
                </el-table-column>
...
import {ApiLoveLiveData, ApiLoveLiveDateList} from "../../types/Api";
...
useHead({
  title: 'BanGDream! Trends',
})

const overview = ref<HTMLElement>()
const color: {[p: string]: string} = {"Poppin' Party": "#FF3377", "Afterglow": "#E53344", "Pastel*Palettes": "#33DDAA", "Roselia": "#3344AA", "Hello, Happy World!": "#FFDD00", "Morfonica": "#33AAFF", "RAISE A SUILEN": "#33CCCC",}
const teams = [{"text":"Poppin' Party","value":"Poppin' Party"},{"text":"Afterglow","value":"Afterglow"},{"text":"Pastel*Palettes","value":"Pastel*Palettes"},{"text":"Roselia","value":"Roselia"},{"text":"Hello, Happy World!","value":"Hello, Happy World!"},{"text":"Morfonica","value":"Morfonica"},{"text":"RAISE A SUILEN","value":"RAISE A SUILEN"}]
const selectedTeams = ref(new Set(["Poppin' Party","Afterglow","Pastel*Palettes","Roselia","Hello, Happy World!","Morfonica","RAISE A SUILEN"]))
...
const getDateInfo = () => {
  request<ApiLoveLiveDateList>(store.getters.getBasePath + '/static/bangdream_trends/date.json?' + Math.random()).then(response => {
    if (response.length) {
...
const getData = () => {
  request<ApiLoveLiveData>(store.getters.getBasePath + '/static/bangdream_trends/' + state.status.date + '.json').then(response => {
...
const accountDataForSunBurst = computed(() => {
  let tmpSunBurstData: { [p: string]: sunBurstType } = {"Poppin' Party":{},"Afterglow":{},"Pastel*Palettes":{},"Roselia":{},"Hello, Happy World!":{},"Morfonica":{},"RAISE A SUILEN":{}}
